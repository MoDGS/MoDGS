...
def run_optical_flows(args):
    ...
    print(basedir)
    subfolder = "1x"
    img_dir = glob.glob(basedir + '/rgb/'+subfolder+'/')[0]  #basedir + '/images_*288'  
    print(img_dir)

    img_path_train = os.path.join(glob.glob(img_dir)[0], '0_00000.png')
    print(img_path_train)
    img_train = cv2.imread(img_path_train)

    interval = 1    
    of_dir = os.path.join(basedir, 'flow_midas%d'%interval)

    if not os.path.exists(of_dir):
        os.makedirs(of_dir)
    
    scene_name = basedir.strip("/").split("/")[-1]
    visual_outs_dir = os.path.join('./Iphone/visual_outs',scene_name)
    if not os.path.exists(visual_outs_dir):
        os.makedirs(visual_outs_dir)
    visual_outs_dir_flow = os.path.join( './Iphone/visual_outs',scene_name,"flow_vis")   
    visual_outs_dir_warped_img = os.path.join('./Iphone/visual_outs',scene_name,"warped_img")   
