...
sys.path.append('./nsff_scripts/core')
...
def motion_segmentation(basedir, threshold):
    ...
    img_dir = os.path.join(basedir, 'rgb')
...
def run_optical_flows(args):
    ...
    print(basedir)
    subfolder = "2x"
    img_dir = glob.glob(basedir + '/rgb/2x')[0]  #basedir + '/images_*288'  
    print(img_dir)

    img_path_train = os.path.join(glob.glob(img_dir)[0], '00000.png')
    if not os.path.exists(img_path_train):
        img_path_train = os.path.join(glob.glob(img_dir)[0], '00000.jpg')
    ...
    interval = 1    
    of_dir = os.path.join(basedir, 'flow_RAFT%d'%interval)
    ...
    scene_name = basedir.strip("/").split("/")[-1]
    visual_outs_dir = os.path.join('./Selfmade/visual_outs',scene_name)
    if not os.path.exists(visual_outs_dir):
        os.makedirs(visual_outs_dir)
    visual_outs_dir_flow = os.path.join( './Selfmade/visual_outs',scene_name,"flow_vis")   
    visual_outs_dir_warped_img = os.path.join('./Selfmade/visual_outs',scene_name,"warped_img")   
    if not os.path.exists(visual_outs_dir_flow):
        os.makedirs(visual_outs_dir_flow)
    if not os.path.exists(visual_outs_dir_warped_img):
        os.makedirs(visual_outs_dir_warped_img)
    with torch.no_grad():
        images = glob.glob(os.path.join(basedir, 'rgb/'+subfolder+'/', '*.png')) + \
                 glob.glob(os.path.join(basedir, 'rgb/'+subfolder+'/', '*.jpg'))
        sorted_imgpaths=sorted(images)
        ...
        for i in range(images.shape[0]-1):
            ...
            try:
                assert image1[0,3,...].min() >254.0 
            except:
                print("path:", sorted_imgpaths[i])
            try:
                assert image2[0,3,...].min() > 254.0
            except:
                print("path:", sorted_imgpaths[i+1])
            ...
            if VIZ:
                if not os.path.exists('./IphoneDataset/viz_flow/'):
                    os.makedirs('./IphoneDataset/viz_flow/')

                if not os.path.exists('./IphoneDataset/viz_warp_imgs'):
                    os.makedirs('./IphoneDataset/viz_warp_imgs')
                ...
                # plt.savefig('./IphoneDataset/viz_flow/'+sorted_imgpaths[i].split("/")[-1][:-4]+'.jpg')
                plt.savefig(os.path.join(visual_outs_dir_flow,sorted_imgpaths[i].split("/")[-1][:-4]+'.jpg'))
                plt.close()
                print(fwd_flow.shape,fwd_flow.dtype )
                warped_im2 = warp_flow(img2, fwd_flow)
                warped_im0 = warp_flow(img1, bwd_flow)

                # cv2.imwrite('./IphoneDataset/viz_warp_imgs/'+sorted_imgpaths[i].split("/")[-1][:-4]+'.jpg', img1[..., ::-1])
                cv2.imwrite(os.path.join(visual_outs_dir_warped_img,sorted_imgpaths[i].split("/")[-1][:-4]+'.jpg'), img1[..., ::-1])
                cv2.imwrite(os.path.join(visual_outs_dir_warped_img,sorted_imgpaths[i].split("/")[-1][:-4]+'_fwd.jpg'), warped_im2[..., ::-1])
                cv2.imwrite(os.path.join(visual_outs_dir_warped_img,sorted_imgpaths[i+1].split("/")[-1][:-4]+'_bwd.jpg'), warped_im0[..., ::-1])
            ...
            np.savez(os.path.join(of_dir, sorted_imgpaths[i].split("/")[-1][:-4]+'_fwd.npz'), flow=fwd_flow, mask=fwd_mask)
            np.savez(os.path.join(of_dir, sorted_imgpaths[i+1].split("/")[-1][:-4]+'_bwd.npz'), flow=bwd_flow, mask=bwd_mask)
