...
def run_optical_flows(args):
    ...
    with torch.no_grad():
        ...
        for i in range(images.shape[0]-1):
            ...
            if VIZ:
                if not os.path.exists('./viz_flow_liu'):
                    os.makedirs('./viz_flow_liu')

                if not os.path.exists('./viz_warp_imgs_liu'):
                    os.makedirs('./viz_warp_imgs_liu')
                ...
                plt.savefig('./viz_flow_liu/%02d.jpg'%i)
                plt.close()

                warped_im2 = warp_flow(img1*fwd_mask[..., np.newaxis], -fwd_flow)
                warped_im1 = warp_flow(img2*bwd_mask[..., np.newaxis], -bwd_flow)
                print(fwd_flow[100,300])
                print(fwd_flow[104,304])
                print(fwd_flow[104,308])
                print(fwd_flow[108,316])
                print(fwd_flow[110,320])
                print(warped_im1.shape, warped_im2.shape)
                print(fwd_mask.shape, fwd_mask.shape)
                cv2.imwrite('./viz_warp_imgs_liu/im_%05d.jpg'%(i), img1[..., ::-1])
                cv2.imwrite('./viz_warp_imgs_liu/im_%05d_fwd.jpg'%(i), warped_im2[..., ::-1])
                cv2.imwrite('./viz_warp_imgs_liu/im_%05d_bwd.jpg'%(i + 1), warped_im1[..., ::-1])
