...
def triangulate(start_points, endpoints, weights):
    """
        Same function as tfg.geometry.representation.ray.triangulate.
        Start_points: (B1, ..., N, 3)
        endpoints: (B1, ..., N, 3)
        weights: (B1, ..., N)
    """
    ...
    for ray_id in range(weights.shape[-1]):
        weights_single = weights[..., ray_id]
        startpoints_single_ray = start_points[..., ray_id, :]
        ...
        cross_product_matrix = torch.stack((zeros, -ray_z, ray_y, ray_z, zeros, -ray_x, -ray_y, ray_x, zeros), dim=-1)
        cross_product_matrix = cross_product_matrix.reshape(list(cross_product_matrix.shape[:-1])+[3,3])
        weights_single = weights_single.unsqueeze(-1)
        left_hand_side = weights_single * cross_product_matrix
        left_hand_side_list.append(left_hand_side)
        dot_product = torch.matmul(cross_product_matrix, startpoints_single_ray.unsqueeze(-1))
        right_hand_side = weights_single * dot_product
    ...
    points = torch.linalg.lstsq(left_hand_side_multi_rays, right_hand_side_multi_rays).solution
